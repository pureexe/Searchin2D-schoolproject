...
int Lst_Insert(int x,int y)
{
    int lst_locate=getInsertAddr(0,lst.size(),x);
    if(lst.empty()||lst[lst_locate].key!=x)
    {
        lst.insert(lst.begin()+lst_locate,zone(x));
    }
    lst[lst_locate].vex.push_back(box(x,y));
    return 0;
}
...
int main()
{
    ...
    {
        sort(a.vex.begin(),a.vex.end(),sortbox);
    }
    /*
    for(struct zone a:lst){
